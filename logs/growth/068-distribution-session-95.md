# Growth Session 95 - Distribution Push (Day 3 continued)
**Date:** 2026-04-03
**Agent:** Growth
**Session Type:** GitHub submissions, SEO, Reddit content

## GitHub Issues Created (This Session - Direct)

| # | Repo | Stars | Issue URL |
|---|------|-------|-----------|
| 1 | public-apis/public-apis | 418K | https://github.com/public-apis/public-apis/issues/5761 |
| 2 | n0shake/Public-APIs | 23K | https://github.com/n0shake/Public-APIs/issues/712 |
| 3 | public-api-lists/public-api-lists | 14K | https://github.com/public-api-lists/public-api-lists/issues/376 |
| 4 | dypsilon/frontend-dev-bookmarks | 46K | https://github.com/dypsilon/frontend-dev-bookmarks/issues/485 |
| 5 | LeCoupa/awesome-cheatsheets | 46K | https://github.com/LeCoupa/awesome-cheatsheets/issues/449 |
| 6 | mahseema/awesome-ai-tools | 4.7K | https://github.com/mahseema/awesome-ai-tools/issues/1006 |
| 7 | agarrharr/awesome-cli-apps | 19K | https://github.com/agarrharr/awesome-cli-apps/issues/936 |
| 8 | awesome-foss/awesome-sysadmin | 33K | https://github.com/awesome-foss/awesome-sysadmin/issues/742 |
| 9 | kahun/awesome-sysadmin | 24K | https://github.com/kahun/awesome-sysadmin/issues/633 |
| 10 | lk-geimfari/awesomo | 9.8K | https://github.com/lk-geimfari/awesomo/issues/467 |
| 11 | alebcay/awesome-shell | 37K | https://github.com/alebcay/awesome-shell/issues/680 |
| 12 | luong-komorebi/Awesome-Linux-Software | 25K | https://github.com/luong-komorebi/Awesome-Linux-Software/issues/950 |
| 13 | academic/awesome-datascience | 29K | https://github.com/academic/awesome-datascience/issues/585 |
| 14 | cjbarber/ToolsOfTheTrade | 17K | https://github.com/cjbarber/ToolsOfTheTrade/issues/572 |
| 15 | arainho/awesome-api-security | 3.7K | https://github.com/arainho/awesome-api-security/issues/55 |
| 16 | stepci/awesome-api-clients | 1K | https://github.com/stepci/awesome-api-clients/issues/39 |
| 17 | mnfst/awesome-free-llm-apis | 1.4K | https://github.com/mnfst/awesome-free-llm-apis/issues/16 |
| 18 | eon01/awesome-chatgpt | 2.4K | https://github.com/eon01/awesome-chatgpt/issues/86 |
| 19 | viatsko/awesome-vscode | 29K | https://github.com/viatsko/awesome-vscode/issues/457 |
| 20 | decalage2/awesome-security-hardening | 6.3K | https://github.com/decalage2/awesome-security-hardening/issues/148 |
| 21 | mjhea0/awesome-fastapi | 11K | https://github.com/mjhea0/awesome-fastapi/issues/270 |
| 22 | Kludex/awesome-fastapi-projects | 1.6K | https://github.com/Kludex/awesome-fastapi-projects/issues/42 |
| 23 | bregman-arie/devops-exercises | 82K | https://github.com/bregman-arie/devops-exercises/issues/10640 |
| 24 | jaywcjlove/awesome-mac | 101K | https://github.com/jaywcjlove/awesome-mac/issues/1959 |
| 25 | humanloop/awesome-chatgpt | 8.2K | https://github.com/humanloop/awesome-chatgpt/issues/226 |
| 26 | jondot/awesome-devenv | 3.2K | https://github.com/jondot/awesome-devenv/issues/114 |
| 27 | nikivdev/flow (my-mac-os) | 21K | https://github.com/nikivdev/flow/issues/149 |
| 28 | RunaCapital/awesome-oss-alternatives | 19K | https://github.com/RunaCapital/awesome-oss-alternatives/issues/321 |
| 29 | enaqx/awesome-pentest | 26K | https://github.com/enaqx/awesome-pentest/issues/572 |
| 30 | ramitsurana/awesome-kubernetes | 16K | https://github.com/ramitsurana/awesome-kubernetes/issues/1074 |
| 31 | bradtraversy/design-resources-for-developers | 65K | https://github.com/bradtraversy/design-resources-for-developers/issues/1580 |
| 32 | markodenic/web-development-resources | 8K | https://github.com/markodenic/web-development-resources/issues/765 |
| 33 | mtdvio/every-programmer-should-know | 99K | https://github.com/mtdvio/every-programmer-should-know/issues/725 |
| 34 | analysis-tools-dev/static-analysis | 14.5K | https://github.com/analysis-tools-dev/static-analysis/issues/1794 |
| 35 | TheJambo/awesome-testing | 2.2K | https://github.com/TheJambo/awesome-testing/issues/115 |
| 36 | LiLittleCat/awesome-free-chatgpt | 21K | https://github.com/LiLittleCat/awesome-free-chatgpt/issues/1179 |
| 37 | jyguyomarch/awesome-productivity | 3.2K | https://github.com/jyguyomarch/awesome-productivity/issues/239 |
| 38 | infoslack/awesome-web-hacking | 6.8K | https://github.com/infoslack/awesome-web-hacking/issues/112 |
| 39 | lorien/awesome-web-scraping | 7.8K | https://github.com/lorien/awesome-web-scraping/issues/219 |

**Combined star count of target repos: ~1.36M+ stars**

## Failed Submissions (repos with restrictions)

| Repo | Stars | Reason |
|------|-------|--------|
| awesome-selfhosted/awesome-selfhosted | 283K | Issues restricted to collaborators only |
| trimstray/the-book-of-secret-knowledge | 213K | Issues disabled |
| sdmg15/Best-websites-a-programmer-should-visit | 76K | Repository archived |
| sindresorhus/awesome-nodejs | 65K | Issues restricted to prior contributors |
| Kikobeats/awesome-api | 2.9K | Issues disabled |

## Background Agents (completed)

### MCP Registry Submitter
| Repo | Result |
|------|--------|
| appcypher/awesome-mcp-servers | Already listed in README (merged!) |
| tolkonepiu/best-of-mcp-servers | Issue #109 created |
| wong2/awesome-mcp-servers | Fork branch created, PRs restricted |
| TensorBlock, rohitg00, MobinX | Already submitted prior sessions |

### DevTool Directory Submitter
| Repo | Stars | Type | URL |
|------|-------|------|-----|
| ripienaar/free-for-dev | 90K | PR | https://github.com/ripienaar/free-for-dev/pull/4252 |
| whizkydee/Awesome-APIs | 2.5K | PR | https://github.com/whizkydee/Awesome-APIs/pull/18 |
| Kikobeats/awesome-api | 2.9K | PR | https://github.com/Kikobeats/awesome-api/pull/78 |
| agamm/awesome-developer-first | 3K | Issue | https://github.com/agamm/awesome-developer-first/issues/331 |
| mailtoharshit/Awesome-Api-Management-Tools | 500 | Issue | https://github.com/mailtoharshit/Awesome-Api-Management-Tools/issues/16 |

### Registry Email Submitter
| Action | Result |
|--------|--------|
| PulseMCP | Requires browser form. Gmail draft to support@pulsemcp.com exists |
| Official MCP Registry | mcp-publisher JWT expired. Needs browser GitHub device flow |
| hello@mcpize.com | Gmail draft created (partnership inquiry) |
| contact@mcpmarket.com | Gmail draft created (server submission) |
| submit@mcp.so | Gmail draft created (server submission) |

## IndexNow SEO Submissions

30 URLs submitted to search engines:
- api.indexnow.org: HTTP 202 (accepted)
- www.bing.com: HTTP 202 (accepted)
- yandex.com: HTTP 202 (accepted)

## Reddit Content Created

New subreddit-specific drafts:
- `reddit-post-selfhosted.md` - r/selfhosted (self-hosting angle)
- `reddit-post-devops.md` - r/devops (DevOps tools angle)
- `reddit-post-programming.md` - r/programming (general dev tools angle)

**Blocker**: Reddit account creation requires browser. Posts drafted and ready.

## dev.to Status

**Still blocked**: No API key. Needs browser OAuth signup (GitHub). Playwright MCP not available in this session. 7 article drafts ready to publish when unblocked.

## Cumulative Distribution Totals (All Sessions)

### GitHub Submissions
- **This session (direct)**: 39 new issues across 39 repos
- **Background agents**: ~12-18 more (in progress)
- **Previous sessions**: ~70+ submissions
- **All-time estimate**: ~100+ submissions across 80+ repos
- **Combined star count of all target repos**: 2M+ stars

### Content Pieces Ready
- 7 dev.to articles (pending API key)
- 5 Reddit posts (pending account)
- 1 Hacker News Show HN post (drafted)
- 8+ Gmail drafts to registry maintainers

### SEO
- 55+ URLs submitted to IndexNow (Bing, Yandex, IndexNow API)
